...
static void client_update_transient_tree(ObClient *self, ObGroup *oldgroup,
                                         ObGroup *newgroup, gboolean oldgtran,
                                         gboolean newgtran, ObClient *oldparent,
                                         ObClient *newparent);
...
void client_startup(gboolean reconfig) {
  client_default_icon = RrImageNewFromData(
      ob_rr_icons, ob_rr_theme->def_win_icon, ob_rr_theme->def_win_icon_w,
...
void client_manage(Window window, ObPrompt *prompt) {
  ...
  ob_debug("Window name: %s class: %s role: %s title: %s", self->name,
           self->class, self->role, self->title);
  ob_debug("Window group name: %s group class: %s", self->group_name,
           self->group_class);
  ...
  */
  ...
  if (!screen_compare_desktops(self->desktop, screen_desktop) && focus_client &&
      client_search_transient(focus_client, self) &&
  ...
  if (ob_state() != OB_STATE_STARTING &&
      ...
      settings->focus != 0 &&
      focus_valid_target(self, self->desktop, FALSE, FALSE, TRUE, TRUE, FALSE,
                         FALSE, settings->focus == 1)) {
    try_activate = TRUE;
  }
  ...
  if (try_activate)
    do_activate = client_can_steal_focus(
        self, settings->focus == 1, (!!launch_time || settings->focus == 1),
        ...
        */
        ob_state() == OB_STATE_RUNNING &&
            (self->type == OB_CLIENT_TYPE_DIALOG ||
             ...
             (!((self->positioned & USPosition) || settings->pos_given) &&
  ...
  */
  ...
  if (self->session)
    ob_debug("  but session requested %d, %d  %d x %d instead, "
             "overriding",
             self->session->x, self->session->y, self->session->w,
             self->session->h);
  ...
  */
  ...
  while (!(cfw >= self->frame->area.width && cfh >= self->frame->area.height)) {
      XResizeWindow(obt_display, self->frame->window,
  cfw>=self->frame->area.width?cfw:++cfw,cfh>=self->frame->area.height?cfh:++cfh);
      usleep(200);
      XFlush(obt_display);
  }
  XFlush(obt_display);
...
ObClient *client_fake_manage(Window window) {
  ...
  client_apply_startup_state(self, self->area.x, self->area.y, self->area.width,
                             self->area.height);
...
void client_unmanage(ObClient *self) {
  ...
  ob_debug("Unmanaging window: 0x%x plate 0x%x (%s) (%s)", self->window,
           self->frame->window, self->class, self->title ? self->title : "");
...
static gboolean client_can_steal_focus(ObClient *self,
  ...
  relative_focused =
      (focus_client != NULL && (client_search_focus_tree_full(self) != NULL ||
  ...
  ob_debug("Want to focus window 0x%x at time %u "
           ...
           self->window, steal_time, launch_time, event_last_user_time,
...
static ObAppSettings *client_get_settings_state(ObClient *self) {
  ...
  for (it = config_per_app_settings; it; it = g_slist_next(it)) {
    ...
    g_assert(app->name != NULL || app->class != NULL || app->role != NULL ||
             app->title != NULL || app->group_name != NULL ||
             app->group_class != NULL || (signed)app->type >= 0);

    if (app->name &&
        !g_pattern_match(app->name, strlen(self->name), self->name, NULL))
      match = FALSE;
    else if (app->group_name &&
             !g_pattern_match(app->group_name, strlen(self->group_name),
                              self->group_name, NULL))
      match = FALSE;
    else if (app->class && !g_pattern_match(app->class, strlen(self->class),
                                            self->class, NULL))
      match = FALSE;
    else if (app->group_class &&
             !g_pattern_match(app->group_class, strlen(self->group_class),
                              self->group_class, NULL))
      match = FALSE;
    else if (app->role &&
             !g_pattern_match(app->role, strlen(self->role), self->role, NULL))
      match = FALSE;
    else if (app->title && !g_pattern_match(app->title, strlen(self->title),
                                            self->title, NULL))
      match = FALSE;
...
static void client_get_shaped(ObClient *self) {
  ...
  if (obt_display_extension_shape) {
    ...
    XShapeQueryExtents(obt_display, self->window, &s, &foo, &foo, &ufoo, &ufoo,
                       &foo, &foo, &foo, &ufoo, &ufoo);
    self->shaped = !!s;
  }
#endif
}
  ...
  */
  ...
  for (it = self->transients; it; it = g_slist_next(it)) {
    ...
    if (!c->transient_for_group)
      client_update_transient_tree(
          c, c->group, c->group, c->transient_for_group, c->transient_for_group,
...
void client_get_mwm_hints(ObClient *self) {
  ...
  if (OBT_PROP_GETA32(self->window, MOTIF_WM_HINTS, MOTIF_WM_HINTS, &hints,
                      &num)) {
    ...
    for (i = 0; i < num; ++i) {
      if (val[i] == OBT_PROP_ATOM(NET_WM_WINDOW_TYPE_DESKTOP))
        ...
        /* prevent this window from getting any decor or
           functionality */
        self->mwmhints.flags &=
            (OB_MWM_FLAG_FUNCTIONS | OB_MWM_FLAG_DECORATIONS);
  ...
  if (XGetWMNormalHints(obt_display, self->window, &size, &ret)) {
    ...
    */
    ...
    ob_debug("Normal hints: min size (%d %d) max size (%d %d)",
             self->min_size.width, self->min_size.height, self->max_size.width,
...
static void client_setup_default_decor_and_functions(ObClient *self) {
  ...
  self->decorations =
      (OB_FRAME_DECOR_TITLEBAR | OB_FRAME_DECOR_HANDLE | OB_FRAME_DECOR_GRIPS |
       OB_FRAME_DECOR_BORDER | OB_FRAME_DECOR_ICON |
       OB_FRAME_DECOR_ALLDESKTOPS | OB_FRAME_DECOR_ICONIFY |
       OB_FRAME_DECOR_MAXIMIZE | OB_FRAME_DECOR_SHADE | OB_FRAME_DECOR_CLOSE);
  self->functions =
      (OB_CLIENT_FUNC_RESIZE | OB_CLIENT_FUNC_MOVE | OB_CLIENT_FUNC_ICONIFY |
       OB_CLIENT_FUNC_MAXIMIZE | OB_CLIENT_FUNC_SHADE | OB_CLIENT_FUNC_CLOSE |
       OB_CLIENT_FUNC_BELOW | OB_CLIENT_FUNC_ABOVE | OB_CLIENT_FUNC_UNDECORATE);
...
void client_update_wmhints(ObClient *self) {
  ...
  if ((hints = XGetWMHints(obt_display, self->window)) != NULL) {
    ...
    if (hints->window_group != (self->group ? self->group->leader : None)) {
      ...
      client_update_transient_tree(
          self, oldgroup, self->group, self->transient_for_group,
          self->transient_for_group, client_direct_parent(self),
...
void client_update_strut(ObClient *self) {
  ...
  if (OBT_PROP_GETA32(self->window, NET_WM_STRUT_PARTIAL, CARDINAL, &data,
                      &num)) {
    if (num == 12) {
      got = TRUE;
      STRUT_PARTIAL_SET(strut, data[0], data[2], data[1], data[3], data[4],
                        data[5], data[8], data[9], data[6], data[7], data[10],
                        data[11]);
    }
    g_free(data);
  }
  ...
  if (!got &&
    ...
    if (num == 4) {
      ...
      STRUT_PARTIAL_SET(strut, data[0], data[2], data[1], data[3], a->y,
                        a->y + a->height - 1, a->x, a->x + a->width - 1, a->y,
                        a->y + a->height - 1, a->x, a->x + a->width - 1);
    }
    g_free(data);
  }
  ...
  if (!img) {
    ...
    if ((hints = XGetWMHints(obt_display, self->window))) {
      if (hints->flags & IconPixmapHint) {
        ...
        xicon = RrPixmapToRGBA(
            ob_rr_inst, hints->icon_pixmap,
            (hints->flags & IconMaskHint ? hints->icon_mask : None), (gint *)&w,
            (gint *)&h, &data);
...
void client_update_icon_geometry(ObClient *self) {
  ...
  if (OBT_PROP_GETA32(self->window, NET_WM_ICON_GEOMETRY, CARDINAL, &data,
                      &num)) {
    if (num == 4)
...
static void client_change_wm_state(ObClient *self) {
  ...
  if (old != self->wmstate) {
    OBT_PROP_MSG(ob_screen, self->window, KDE_WM_CHANGE_STATE, self->wmstate, 1,
                 0, 0, 0);
...
gboolean client_has_parent(ObClient *self) { return self->parents != NULL; }
...
static void client_apply_startup_state(ObClient *self, gint x, gint y, gint w,
  ...
  */
  ...
  ob_debug("placed window 0x%x at %d, %d with size %d x %d", self->window, x, y,
           w, h);
...
void client_try_configure(ObClient *self, gint *x, gint *y, gint *w, gint *h,
  ...
  *w = MIN(*w, (gint)G_MAXUSHORT - self->frame->size.left -
                   self->frame->size.right);
  *h = MIN(*h, (gint)G_MAXUSHORT - self->frame->size.top -
                   self->frame->size.bottom);
...
void client_configure(ObClient *self, gint x, gint y, gint w, gint h,
  ...
  send_resize_client = ((!user && resized) ||
                        (user && (final || (resized && config_resize_redraw))));

  /* if the client is enlarging, then resize the client before the frame */
  if (send_resize_client && (w > oldclient.width || h > oldclient.height)) {
    XMoveResizeWindow(obt_display, self->window, self->frame->size.left,
                      self->frame->size.top, MAX(w, oldclient.width),
  ...
  if (!user || final) {
    ...
    POINT_SET(self->root_pos,
              self->frame->area.x + self->frame->size.left - self->border_width,
...
void client_close(ObClient *self) {
  ...
  if (!self->delete_window)
    ...
    OBT_PROP_MSG_TO(self->window, self->window, WM_PROTOCOLS,
                    OBT_PROP_ATOM(WM_DELETE_WINDOW), event_time(), 0, 0, 0,
                    NoEventMask);
...
static void client_prompt_kill(ObClient *self) {
  ...
  if (!self->kill_prompt) {
    ObPromptAnswer answers[] = {{0, OB_KILL_RESULT_NO},
    ...
    if (client_on_localhost(self)) {
      ...
      m = g_strdup_printf(
          _("The window \"%s\" does not seem to be responding.  Do you want to "
            "force it to exit by sending the %s signal?"),
          title, sig);
      y = _("End Process");
    } else {
      m = g_strdup_printf(_("The window \"%s\" does not seem to be responding. "
                            " Do you want to disconnect it from the X server?"),
                          title);
      y = _("Disconnect");
    }
    ...
    self->kill_prompt =
        prompt_new(m, NULL, answers, sizeof(answers) / sizeof(answers[0]),
...
void client_kill(ObClient *self) {
  ...
  if (client_on_localhost(self) && self->pid) {
    ...
    if (self->kill_level == 0) {
      ob_debug("killing window 0x%x with pid %lu, with SIGTERM", self->window,
               self->pid);
      ...
      ob_debug("killing window 0x%x with pid %lu, with SIGKILL", self->window,
               self->pid);
...
static void client_bring_windows_recursive(ObClient *self, guint desktop,
                                           gboolean helpers, gboolean modals,
                                           gboolean iconic) {
  GSList *it;
...
gboolean client_focused(ObClient *self) { return self == focus_client; }
...
static void detect_edge(Rect area, ObDirection dir, gint my_head, gint my_size,
                        gint my_edge_start, gint my_edge_size, gint *dest,
                        gboolean *near_edge) {
...
void client_find_edge_directional(ObClient *self, ObDirection dir, gint my_head,
                                  gint my_size, gint my_edge_start,
                                  gint my_edge_size, gint *dest,
                                  gboolean *near_edge) {
  GList *it;
  Rect *a;
  Rect dock_area;
  ...
  *near_edge = TRUE;
  ...
  for (i = 0; i < screen_num_monitors; ++i) {
    ...
    detect_edge(*area, dir, my_head, my_size, my_edge_start, my_edge_size, dest,
                near_edge);
...
gboolean client_has_relative(ObClient *self) {
  return client_has_parent(self) || client_has_group_siblings(self) ||
